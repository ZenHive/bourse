defmodule Bourse.Journeys.Trader.BinanceusdmTest do
  ...
  # 0.002 BTC at a 10% under-market bid sits above MIN_NOTIONAL 50 USDT
  # (observed live 2026-08-28: 0.002 * ~71_865 ≈ 143 USDT).
  @amount 0.002
  # BTCUSDT PRICE_FILTER tickSize 0.1; resting bid sits 10% under market,
  # inside PERCENT_PRICE_BY_SIDE (observed 2026-08-28).
  @price_decimals 1
  @resting_ratio 0.9
  # 1 BTC at the same resting price was accepted on this demo wallet
  # (high leverage); 10 and 100 BTC were refused with -2019.
  @margin_exceeding_amount 100.0
  ...
  describe "a trader's day" do
    test "survey the market, place a resting limit buy, track it, cancel it" do
      ...
      assert market, "#{@symbol} missing from loaded binanceusdm markets"
      ...
      # Observed live 2026-08-28: GET /fapi/v1/ticker/24hr fills last/high/low
      # and timestamp; bid and ask are nil. The book is the spread authority.
      ...
      {:ok, placed} =
        Bourse.create_order(exchange, @symbol, "limit", "buy", @amount,
          price: price,
          timeInForce: "GTC",
          newClientOrderId: unique_client_order_id("tbusdm")
        )
  ...
  describe "a trader watching the private order stream" do
    test "the order's own lifecycle arrives on the stream, place and cancel" do
      ...
      try do
        # connect/3 issued the listen key over REST and opened demo-fstream
        # with it in the URL; there is no later handshake and no channel to
        # subscribe to. A wrong key still reports connected and delivers
        # nothing — success is ORDER_TRADE_UPDATE about our order id.
        assert %{pattern: :listen_key} = ws.auth
        assert ws.url =~ "demo-fstream.binance.com"
        assert ws.url =~ "listenKey="
        assert ws.url =~ "ORDER_TRADE_UPDATE"
        ...
        try do
          # Observed live 2026-08-28 on wss://demo-fstream.binance.com:
          # envelope e "ORDER_TRADE_UPDATE"; o.X/x "NEW", o.ps "BOTH"
          # (One-way mode), o.c the newClientOrderId. Authority: Binance
          # USD-M Event Order Update
          # (https://developers.binance.com/docs/derivatives/usds-futures/user-data-streams).
          ...
          # o.c still the original newClientOrderId, o.z "0".
  ...
  describe "orders the venue rejects" do
    test "an oversized notional is refused with binanceusdm's own error" do
      exchange = sandbox_exchange!(@venue)
      {:ok, book} = Bourse.fetch_order_book(exchange, @symbol)
      assert [[best_bid, _bid_size] | _] = book.bids
      price = Float.round(best_bid * @resting_ratio, @price_decimals)
      ...
      # Observed live 2026-08-28: POST /fapi/v1/order with quantity 100
      # (valid LOT_SIZE, notional far above this wallet's margin) answers
      # code -2019, "Margin is insufficient." Quantity 10000 trips -4005
      # "Quantity greater than max quantity." first; 1 BTC was accepted.
      # Authority: Binance USD-M error codes —2019 MARGIN_NOT_SUFFICIEN
  ...
  # USD-M user-data events arrive as {:websocket_message, %{"e" =>
  # "ORDER_TRADE_UPDATE", "o" => order}}. Order id `i` is an integer on the
  # wire; REST returns it as a string. ACCOUNT_UPDATE frames on the same
  # socket are skipped.
    ...
    if left <= 0 do
      ...
      receive do
        {:websocket_message, %{"e" => "ORDER_TRADE_UPDATE", "o" => order}} ->
          if order_event?(order, order_id, status) do
            order
          else
  ...
  defp order_event?(order, order_id, status) do
    order["X"] == status and to_string(order["i"]) == to_string(order_id)
  end
end
