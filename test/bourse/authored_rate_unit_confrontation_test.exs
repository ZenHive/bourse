defmodule Bourse.AuthoredRateUnitConfrontationTest do
  ...
  test "every authored rate-like slot has a Task 594 venue-unit confrontation" do
    for venue <- @venues,
        path <- rate_paths(Bourse.Spec.load!(venue)) do
  ...
  test "every venue funding-rate slot cites a provider-owned unit statement or arithmetic" do
    for venue <- @venues,
        path <- rate_paths(Bourse.Spec.load!(venue)),
  ...
  defp rate_paths(spec) do
    field_map_paths =
      spec
      |> get_in(~w(normalization field_maps))
      |> collect_field_map_paths(~w(normalization field_maps))

    static_fee_paths = collect_static_fee_paths(spec["fees"], ["fees"])

    Enum.sort(field_map_paths ++ static_fee_paths)
  end

  defp collect_field_map_paths(%{} = map, path) do
    Enum.flat_map(map, fn {key, value} ->
      child_path = path ++ [key]
      current = if key in @rate_keys and (is_map(value) or is_nil(value)), do: [format_path(child_path)], else: []
    ...
    Enum.flat_map(map, fn {key, value} ->
      ...
      cond do
        ...
        key in ~w(maker taker) and is_list(value) and value != [] ->
          [format_path(child_path ++ [:all, "rate"])]
