...
| Task 327 | 🔶 | 🎁 **authored_specs** · Work the prod-verification ledger: close deferred tier-1 confirmations with prod keys / real market state [D:3/B:6/U:4 → Eff:1.67?] 🚀 ⛔ Ledger review 2026-09-15; historical observations, not fresh live probes. External prerequisites recorded by venue: OKX International needs production read access for demo-unhosted funding/savings surfaces and populated transaction history; Binance Spot/USD-M/COIN-M need product-authorized production access or matching position/history state; Derive needs unwindable demo liquidity and funding-bearing positions; Deribit needs matching account state or isolated/disposable accounts for account-wide operations; Hyperliquid needs bridge/withdrawal history or an eligible vault; Lighter records an account-provisioning issue and missing fee-bearing/withdrawal/liquidation rows. These do not block every ledger entry: Bybit spot coverage and the Alpaca private-stream configuration are implementation gaps, while Bybit settlement seeded for 2026-08-28 and other dated state claims require rechecking before treating them as current blockers. The ledger remains the per-case source of truth. Production conversions, withdrawals and other operator-gated mutations still require explicit authorization. |
...
| Task 349 | 🔶 | 🎁 **authored_specs** · 🐛 Determine and carve Binance sapi asset/dust encoding from authoritative evidence [D:3/B:5/U:4 → Eff:1.5?] 🚀 ⛔ Rechecked 2026-09-15: provider documentation https://developers.binance.com/en/docs/catalog/core-trading-wallet/api/rest-api/asset#dust-transfer and the official binance-connector-python wallet AssetApi.dust_transfer both specify one comma-separated asset string (asset=BTC,USDT). The endpoint-specific urlencodeCommaSeparatedArray recipe already exists in priv/venues/binance/authored/venue.json, with signing assertions in test/bourse/signing/hmac_recipe_test.exs. Encoding discovery is therefore not the remaining blocker. Successful production execution remains unverified: docs/prod-verification-ledger.md records only a historical non-mutating dust preview; conversion requires explicit operator approval for the named assets and a trade-enabled production key. No production mutation or current key-permission probe was performed in this recheck. The endpoint offers no amount cap. Historical key restrictions are not freshly verified. |
