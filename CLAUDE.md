...
- **Bybit** — `BYBIT_TESTNET_API_KEY/SECRET` is an **AI sub-account credential** (`sub_member_id 107065959`, re-provisioned 2026-08-24) and is **writable**: `GET /v5/user/query-api` reports `readOnly: 0` with `SpotTrade` / `ContractTrade [Order, Position]` / `OptionsTrade`, and a live `POST /v5/order/create` + `/v5/order/cancel` round-trip returns `retCode 0`. 🚨 **Writable means derivatives only:** every SPOT create and `POST /v5/asset/exchange/convert-execute` answer business error 10024 (regulatory) on this same key, seconds apart from an accepted linear create — so order-state population and the order-identity contract cases run under `category=linear`, never spot. The **earlier** testnet key was read-only (10024 on any signed create, region-restricted); that is why trade evidence was routed to DEMO, and the DEMO path still works: `BYBIT_DEMO_API_KEY/SECRET`, host `https://api-demo.bybit.com` — which is **not** `sandbox: true` (that's testnet); pass `base_url:` on the call. Requests omitting `category` fail with 10032.
  - 🚨 **The testnet web UI cannot mint an API key, and the reason is the account, not the dialog** — driven live 2026-08-24, `POST /x-api/api/newAdd` answers `ret_code 10024`, *"not available to you due to regulatory restrictions"*, with a `KYC_PROMPT_TOAST` config and `kycStatus: 0`. That is the same 10024 the earlier testnet HMAC key returned on any signed create, so the main testnet member is regulatory-restricted from the API product itself; no permission set, captcha retry or browser fixes it, and the open support ticket is the only route. **Separately, never use the `/en-GB/` locale**: it serves the UK/Archax compliance wall, whose `RESTRICTED_PROMPT_TOAST` list carries `MAIN_API` and every `MAIN_DERIVATIVES*` entry — the UI then shows no perps at all and the key form offers only SPOT. On `/en/` that list collapses to six unrelated entries and the full derivatives permission surface appears, but 10024 still stands. The working route is Bybit's **AI sub-account OAuth flow**, and it has enough sharp edges to be worth writing down; the vendor spec is [the bybit-exchange/skills OAuth module](https://raw.githubusercontent.com/bybit-exchange/skills/main/modules/oauth.md), whose own instructions are wrong in two places:
