...
that suite asserted — and it is not the whole REST-read surface: the 410-case
...
- **Lighter** — DEX (zk perp), not an HMAC pair: `LIGHTER_TESTNET_API_KEY_INDEX` (0–255), `LIGHTER_TESTNET_ACCOUNT_INDEX`, `LIGHTER_TESTNET_API_PRIVATE_KEY` (40-byte hex). Signing is zk-Schnorr through the supervised first-party helper (`Bourse.Signing.Lighter` + `native/lighter_signer/`) — there is no in-Elixir signer. `sandbox: true` selects the testnet host **and** chain id 300 (mainnet is 304; the chain id is part of the signed payload, so a mainnet-chain signature is rejected on testnet). Private reads need an `auth_deadline` and `account_index`; writes need a caller-supplied `nonce` from `public_get_nextnonce` plus a `client_order_index`. Only `limit` orders are supported.
  - 🚨 **`20013 "invalid auth: couldnt find account"` almost never means the account is gone — it means the configured `LIGHTER_TESTNET_API_KEY_INDEX` is not where the key sits, and the message names the *account* anyway.** Every session before 2026-09-15 read it as a testnet reset and ran `mix bourse.provision_lighter`, which mints a key at a NEW index and breaks every other machine pinned to the old one — that loop is why this "kept coming back". The wallet is already provisioned: `LIGHTER_TESTNET_ACCOUNT_INDEX` names the account whose L1 address is `$HYPERLIQUID_TESTNET_API_KEY`, and `LIGHTER_TESTNET_API_PRIVATE_KEY` derives exactly the key registered at `LIGHTER_TESTNET_API_KEY_INDEX`. **Do not provision.** `Bourse.Lighter.CredentialCheck.run/1` asks the venue which indices the account actually carries and prints the export that fixes it; `test/test_helper.exs` runs it at startup, and the provisioning task refuses to mint a second key.
  - 🚨 **A long-lived process env beats `~/.secrets`.** A session that started before the file was corrected keeps the stale export, so re-editing the file does nothing for it — compare `echo "$LIGHTER_TESTNET_ACCOUNT_INDEX"` against `zsh -l -c 'echo $LIGHTER_TESTNET_ACCOUNT_INDEX'` before touching anything. The same drift exists per machine: the harness server has its own `~/.secrets`, invisible from here.
