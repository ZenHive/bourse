...
# state process-globally under a key derived from the exchange id. The suite
# drives the same eleven ids through failure paths from dozens of unrelated
...
# that venue in the meantime fails with `circuit_open` before it ever reaches the
# venue — a red that names neither the venue nor the test that melted it.
#
